...
Aap bol hi rahe hote ho, tab tak kaam shuru ho jaata hai: "youtube kholo…" bolte hi YouTube khul
jaata hai, aur "…aur lofi search karo" key chhodte hi chalta hai.
...
- **Right Alt dabake rakho → bolo → chhod do.** Neeche screen pe ek chhota bar dikhta hai jisme
  aapki baat aur action aata hai. Beep = sunna shuru / band.
...
Ek saath do kaam: "youtube kholo aur lofi search karo", "notepad kholo aur hello world likho".
...
**Bolte-bolte action:** key dabe rehte hue har ~0.45 s me speech dobara transcribe hoti hai. Jo command
poori ho chuki hai (e.g. Hinglish me verb aa gaya: "youtube *kholo*") wo turant chalti hai. Search /
type ka text key chhodne pe final hota hai, taaki query beech me na kate.
...
Tests: 170+ Hindi/English/Hinglish phrases ka parser table, streaming/partial behaviour,
confirm/pick flows, aur ek local test page pe asli Chromium (click, ordinal results, numbered
badges, typing, scroll, tabs). Laya tests `models/laya` hone pe chalte hain.

## Status

Core logic (parser, controller, browser actions, models offline load) automated tests se check
hai. Windows-specific parts (hotkey hook, system actions, `setup.ps1`) Win32 APIs pe likhe gaye
hain; `setup.ps1` PowerShell parser se validate hai. Koi problem aaye to `logs/voicemode.log` ke
saath issue kholo.
